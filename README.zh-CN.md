...
- **策略引擎** — 单笔交易和每日美元价值上限、滑点限制、杠杆上限、白名单、操作开关、分级审批（自动批准 / 用户确认 / 硬性阻止）
- **非托管密钥存储** — 生成钱包采用沙米尔秘密分享（2-of-3），导入钱包使用 AES-256-GCM 加密，密码短语会话支持可配置 TTL
- **多智能体安全** — 代理进程/守护进程架构允许多个智能体（Claude Desktop、Cursor、Codex 等）共享同一密钥存储和密码短语会话，不会出现状态分裂问题
- **多链** — Solana、Ethereum、Base、Arbitrum、Optimism、Polygon、BNB Chain、Avalanche、Monad、Bitcoin（+ 可配置的 EVM 链和测试网）
- **智能体无关** — 适用于任何支持 MCP 的客户端；为 10+ 智能体/编辑器提供一键配置模板
- **防诈骗保护** — 可选的签名诈骗地址黑名单、OFAC SDN 检查、收款人/合约白名单
- **验证** — 发布资产使用 Sigstore Cosign 签名，生成 SBOM（SPDX-JSON）

## 安装

Seashail 通过 GitHub Releases 分发，也可以从源码构建。你的智能体通过 `seashail mcp` 以 MCP stdio 服务器方式运行它。
...
如果你不想安装二进制文件，可以在你的智能体 MCP 配置中引用以下启动器。它们会自动安装并运行 Seashail。
...
**Cline、Continue、JetBrains 或任何 MCP 客户端** — 参见下方 [智能体集成](#智能体集成)或[完整智能体设置指南](https://seashail.com/docs/guides/agents)。
...
首次连接时，Seashail 会自动创建默认钱包（EVM、Solana、Bitcoin 地址），你的智能体可以立即查询余额 —— 无需任何提示。

要创建具有更强恢复选项的钱包，可以让你的智能体调用 `create_wallet`。这会使用 MCP 交互确认来：

- 设置密码短语（至少 8 个字符，不存储在任何地方）
- 显示并确认离线备份密钥分片（沙米尔秘密分享 2-of-3）
- 接受免责声明

密钥材料永远不会离开 Seashail 进程。智能体只能接收工具输出（余额、报价、交易哈希）。

### 4. 验证

在你的智能体中尝试：

- `get_capabilities` — 健康检查：查看已配置的 RPC、兑换后端、链
- `list_wallets` — 查看你的钱包和地址
- `get_balance` — 查看余额

## 架构

Seashail 使用代理-守护进程架构，允许多个智能体安全地共享单一密钥存储和策略状态。
...
1. **多智能体共享状态** — 在 Claude Desktop 中创建的钱包会出现在 Cursor 中；密码短语只需输入一次即可在所有智能体中解锁；策略变更全局生效
2. **并发访问安全** — 独占文件锁防止并发写入导致密钥存储损坏
3. **密码短语会话缓存** — 解锁一次，所有智能体即可使用（可配置 TTL，默认 1 小时）
...
| `buy_nft`           | 通过交易市场交易载荷购买 NFT（Blur、Magic Eden、OpenSea、Tensor） |
| `sell_nft`          | 通过交易市场交易载荷出售/挂单 NFT                                 |
| `bid_nft`           | 通过交易市场交易载荷出价/报价                                     |
...
Seashail 假定智能体进程可能是恶意的或已被攻破的。二进制程序本身是安全边界。
...
| 威胁             | 缓解措施                                                              |
| ---------------- | --------------------------------------------------------------------- |
| 恶意智能体       | 策略引擎 + 分级审批 + 白名单 + 操作开关                               |
| 从日志窃取密钥   | MCP 交互确认（密钥不会出现在智能体对话中）；工具 schema 明确拒绝密钥参数 |
| 状态分裂         | 独占文件锁；单例守护进程                                              |
...
| 智能体访问 | 策略管控的 MCP 工具              | 直接签名     | 使用提供商密钥的 API |
| 恢复方式   | 沙米尔秘密分享 2-of-3 + 密码短语 | 助记词       | 提供商流程           |
| 风险模型   | 智能体受策略约束                 | 用户逐笔验证 | 信任第三方           |
...
Seashail 适用于任何支持 MCP 的智能体。对于[快速开始](#快速开始)中未列出的智能体：
...
多个智能体通过单例守护进程安全共享相同的钱包、密码短语会话和策略计数器。每个 `seashail mcp` 进程都是一个轻量级代理进程。
...
| 策略硬性阻止          | 超过美元上限     | 检查 `get_policy`，调整限额                 |
