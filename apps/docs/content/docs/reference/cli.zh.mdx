...
> 没有 `seashail wallet` 命令。钱包操作仅通过 MCP 工具进行。通过 MCP 智能体使用 `create_wallet`、`import_wallet` 和其他钱包工具。
...
- 有关代理/守护进程关系的详细信息，请参阅[架构](/zh/docs/reference/architecture)。
...
- 守护进程在 `$DATA_DIR/seashail-daemon.lock` 处持有独占文件系统锁，以防止脑裂状态。
...
安装或管理 OpenClaw 插件集成。这为 OpenClaw 提供一流的 Seashail 工具支持。
