...
- **库存查看：** 查看你拥有的 NFT
- **直接转移：** 将 NFT 发送到另一个地址
- **交易市场交易：** 通过交易封装在 NFT 市场上买入、卖出或出价

交易市场交易使用**交易封装**——代理（或已配置的适配器）构建特定交易市场的交易，Seashail 在策略评估后签名。
...
交易市场操作（`buy_nft`、`sell_nft`、`bid_nft`）需要**交易封装**：

1. 代理（或适配器）获取交易市场的挂单/报价数据
2. 代理构建交易（或提供 `asset` 对象供已配置的适配器获取）
...
- 或者提供 `asset` 对象让交易市场适配器获取封装
- 策略评估适用（`enable_nft`、`max_usd_per_nft_tx`）
- Solana 交易市场封装**始终强制确认**（无论策略层级如何都需要用户确认）
...
- 高交易量的 Ethereum 交易市场
- 封装：带有 `to`、`data`、`value_wei` 的 EVM 交易
- 交易在广播前进行模拟
- 诈骗地址黑名单适用于合约地址
...
- Solana 封装：带 `allowed_program_ids` 的 `tx_b64`
- EVM 封装：`to`、`data`、`value_wei`
- Solana 封装强制确认（需要用户确认）
...
- 封装：带 `allowed_program_ids` 的 `tx_b64`
- Solana 封装强制确认（需要用户确认）
- 以支持压缩 NFT 著称
...
- 超过上限的交易会被拦截
- 低于上限的交易可能自动审批或需要确认，具体取决于分级审批设置
...
- **封装二元性：** 代理可以直接提供封装，或已配置的适配器通过 `asset` 字段获取
- **Solana 交易市场封装始终强制确认**——无论策略层级如何都需要用户确认
- 相关内容：[钱包指南](/zh/docs/guides/wallets)，了解钱包地址管理

## 另见

- [故障排除](/zh/docs/troubleshooting)，了解策略错误和交易市场封装问题的解决方案
