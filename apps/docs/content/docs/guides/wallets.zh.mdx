...
Seashail 的设计确保 agent 进程永远不会收到密钥材料。
...
- 密钥分片 2：默认使用机器密钥加密（方便上手），或在 passphrase 下重新加密以实现可移植性（可选启用）
...
导入的私钥/助记词使用 AES-256-GCM 静态加密，加密密钥由你的 passphrase 派生（Argon2id + HKDF 子密钥）。

## 密码短语会话

为了使自动化可行，Seashail 将 passphrase 派生的密钥缓存在内存中，持续时间可配置（`passphrase_session_seconds`）。

无人值守环境可以选择通过环境变量提供 passphrase：
...
- `create_wallet`（通过 MCP 交互确认提示输入 passphrase）
...
- 不要将 `secret` 作为工具参数传递。Seashail 在设计上会拒绝它，因为工具参数经常出现在 agent 日志中。
