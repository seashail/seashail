---
title: 快速开始
description: 安装 Seashail，连接你的 agent，创建你的第一个钱包。
---
...
其他所有 agent（Cline、Continue、JetBrains 以及任何 MCP 客户端），参见[连接你的 Agent](../guides/agents)。
...
首次连接时，Seashail 会自动创建一个默认钱包，以便你的 agent 可以立即查询地址和余额 — 无需任何提示。此钱包使用机器本地密钥加密，这意味着它与创建它的设备绑定。

要创建一个具有更强恢复选项的额外钱包，让你的 agent 调用 `create_wallet` 工具。此操作使用 MCP 交互确认来：

- 设置密码短语（至少 8 个字符，不存储在任何地方）
- 显示并确认离线备份密钥分片（沙米尔秘密分享 2-of-3）
- 接受免责声明

密钥材料永远不会离开 Seashail 进程。Agent 只接收工具输出（余额、报价、交易哈希等）。有关安全详情，参见[安全模型](../guides/security-model)。

**关键术语：**

- **交互确认：** MCP 协议中用于用户提示（密码短语、确认操作）的机制，数据不会发送给 agent。参见[术语表](../glossary)。
